...
class BERT_Arch(nn.Module):
    ...
    def __init__(self, bert):
      ...
      self.num_comet_features = 10
      self.num_emotions = 8
      # for comet word2vec embeddings
      self.comet = nn.Linear(self.embedding_dim*(self.num_comet_features),256)

      self.emo = nn.Linear(self.num_emotions,256)
    ...
    def forward(self, sent_id, mask):
      ...
      # comet
      x_com = self.get_comet(sent_id)

      x_com = self.comet(x_com)

      x_com = x_com.append(self.emo(self.get_emo(sent_id)))
